...
# Updated by wcx, 2026-4-17: to make it suitable for casci input
...
class SISO():
    def __init__(self, title, mc, statelis=None, amfi=False, save_mag=True, save_Hmat=False, save_old_Hal=False, verbose=5):        
        ...
        if statelis is not None:
            ...
            try:
                ...
                # 如果是标准 PySCF CASCI, 自动根据 nroots 和 spin 构建 statelis
                nroots = getattr(mc.fcisolver, 'nroots', 1)
                spin = mc.fcisolver.spin
                # 假设 S = spin (注意 PySCF spin = 2S)
                S_idx = int(spin) 
                max_S = S_idx + 1
                self.statelis = np.zeros(max_S, dtype=int)
                self.statelis[S_idx] = nroots

        self.Smax = np.shape(self.statelis)[0]
        self.Slis = np.nonzero(self.statelis)[0]

        # for casscf, e_states ; for casci, e_tot
    ...
    def make_full_trans_dm(self):
        ...
        mc = self.mc
        if hasattr(mc.fcisolver, 'undo_state_average'):
            unmixed_fcisolver = mc.fcisolver.undo_state_average() # unmix for calling trans_rdm1
        else:
            unmixed_fcisolver = mc.fcisolver

        ci_vecs = mc.ci if isinstance(mc.ci, (list, tuple)) else [mc.ci]

        for S in self.Slis:
            unmixed_fcisolver.spin = S
            state_index = self.casscf_state_idx[S]
    ...
    def calc_z(self):
        ...
        # For multi-state calculations, use the first state's CI vector
        if hasattr(self.mc.fcisolver, 'nroots') and self.mc.fcisolver.nroots > 1:
            sodm1 = self.mc.make_rdm1(ci=self.mc.ci[0])
        else:
    ...
    def calc_Y(self):
        ...
        from embed_sim.spin_utils import unpack_nelec

        for S1, S2 in itertools.product(range(0, self.Smax), range(0, self.Smax)):
            S1 = int(S1)
            S2 = int(S2)
            if self.statelis[S1] == 0 or self.statelis[S2] == 0:
                continue
            ...
            if S1 == S2: # z_0 s_0
                for I1, I2 in itertools.product(self.casscf_state_idx[S1],
                    ...
                    nelec_tmp = unpack_nelec(mc.nelecas, spin=S1)
                    t_dm1 = mc.fcisolver.trans_rdm1s(ci_vecs[I2], ci_vecs[I1], mc.ncas, nelec_tmp) # shape (2, ncas, ncas)
                    
                    from sympy.physics.wigner import wigner_3j
                    w3j_0 = float(wigner_3j(S2/2, 1, S1/2, -S2/2, 0, S1/2))
                    if abs(w3j_0) > 1e-8:
                        for m in range(0, 3): # -1, 0, 1
                            Y[m, I1, I2] = 1 / w3j_0 * lib.einsum('ij,ij->', self.z[m], 0.5 * (t_dm1[0] - t_dm1[1]))
            ...
            for MS1, MS2 in itertools.product(range(-S1, S1+1, 2),
                ...
                if np.abs(MS1 - MS2) <= 2:
                    ...
                    if S1 == S2 and MS1 == MS2:
                        self.SOC_Hamiltonian[np.ix_(self.siso_state_idx[S1, MS1], self.siso_state_idx[S2, MS2])] += np.diag(self.e_states[self.casscf_state_idx[S1]])
        if self.save_Hmat:
    ...
    def solve(self, nprint=4, ncomp=10):
        ...
        for i in range(0, np.min((nprint, self.nstates))): # print 10 biggest coefficients and corresponding spin states
            ...
            for j in range(0, np.min((ncomp, self.nstates))):
